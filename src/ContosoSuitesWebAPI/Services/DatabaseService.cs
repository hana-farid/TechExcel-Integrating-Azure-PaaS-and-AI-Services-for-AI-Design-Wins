...
public class DatabaseService(string connectionString) : IDatabaseService
{
    ...
    public async Task<IEnumerable<Booking>> GetBookingsByHotelAndMinimumDate([Description("The ID of the hotel")] int hotelId, [Description("the minimum date of the booking")] DateTime dt)
    {
    ...
    /// </summary>
    [KernelFunction]
    [Description("Get bookings that are without hotel rooms.")]
    public async Task<IEnumerable<Booking>> GetBookingsMissingHotelRooms()
    {
        var sql = "SELECT b.BookingID, b.CustomerID, b.HotelID, b.StayBeginDate, b.StayEndDate, b.NumberOfGuests FROM dbo.Booking b WHERE NOT EXISTS ( SELECT 1 FROM dbo.BookingHotelRoom h WHERE b.BookingID = h.BookingID)";
        using var conn = new SqlConnection(
            connectionString: connectionString!
        );
        conn.Open();
    ...
    /// </summary>
    [KernelFunction]
    [Description("Get bookings that have multiple hotel rooms booked.")]
    public async Task<IEnumerable<Booking>> GetBookingsWithMultipleHotelRooms()
    {
        var sql = "SELECT b.BookingID, b.CustomerID, b.HotelID, b.StayBeginDate, b.StayEndDate, b.NumberOfGuests FROM dbo.Booking b WHERE (SELECT COUNT(1) FROM dbo.BookingHotelRoom h WHERE b.BookingID = h.BookingID) > 1";
        using var conn = new SqlConnection(
            connectionString: connectionString!
        );
        conn.Open();
