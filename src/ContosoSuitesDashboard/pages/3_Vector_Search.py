...
def handle_query_vectorization(query):
    ...
    response = requests.get(f"{api_endpoint}/Vectorize", params={"text": query}, timeout=100, verify=False)
...
def handle_vector_search(query_vector, max_results=5, minimum_similarity_score=0.8):
    ...
    response = requests.post(f"{api_endpoint}/VectorSearch", data=query_vector, params={"max_results": max_results, "minimum_similarity_score": minimum_similarity_score}, headers=headers, timeout=100, verify=False)
    return response
