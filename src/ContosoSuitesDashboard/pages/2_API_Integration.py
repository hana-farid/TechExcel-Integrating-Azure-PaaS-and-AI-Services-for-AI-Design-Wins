...
def get_hotels():
    ...
    response = requests.get(f"{api_endpoint}/Hotels", timeout=100)
    return response
...
def get_hotel_bookings(hotel_id):
    ...
    api_endpoint = st.secrets["api"]["endpoint"]
    response = requests.get(f"{api_endpoint}/Hotels/{hotel_id}/Bookings", timeout=100)
    return response
...
def invoke_chat_endpoint(question):
    ...
    api_endpoint = st.secrets["api"]["endpoint"]
    response = requests.post(f"{api_endpoint}/Chat", data={"message": question}, timeout=100)
    return response
